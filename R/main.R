...
formatNS <- function(ns) {
    ...
    # A prefix is written into the XPath - into the node tests the
    # translator emits, and into the descendant filter formatNSPrefix()
    # builds - so it has to be a name the evaluator can parse. That is
    # the test is_ncname() makes, the same one the translator applies to
    # a prefix written in the selector, so both entry points accept the
    # same prefixes, non-ASCII ones included.
    badNames <- nsNames[!vapply(nsNames, is_ncname, logical(1))]
    if (length(badNames))
